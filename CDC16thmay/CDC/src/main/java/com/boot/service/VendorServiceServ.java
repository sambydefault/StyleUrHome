...
public class VendorServiceServ {
	...
	public List<ConsumerServicePrice> getVendorItems(String userEmail, Vendor vendor, List<ConsumerServicePrice> listConsumerPrice,
		...
		List<ConsumerServicePrice> returnListConsumerPrice = new ArrayList<ConsumerServicePrice>();
		...
		listConsumerPrice = consumerServicePriceRepository
				.findAllIemsByVendorCategory(properties.get("STATUS_QUOTE").toString());

		for (ConsumerServicePrice objConsumerServicePrice : listConsumerPrice) {
			for (String vendorSubCategory : listVendorCategory) {
				if (objConsumerServicePrice.getProduct().getProductSubCategory().equals(vendorSubCategory))
					returnListConsumerPrice.add(objConsumerServicePrice);
			}
		}
