...
public class VendorService {
	...
	public List<ConsumerPrice> getVendorItems(String userEmail, Vendor vendor, List<ConsumerPrice> listConsumerPrice,
		...
		List<ConsumerPrice> returnListConsumerPrice = new ArrayList<ConsumerPrice>();
		/*
		 * for (int i = 0; i < listVendor.size(); i++) {
		 * listProduct.add(listVendor.get(i).getProduct().getProductid()); }
		 */
		List<String> listVendorCategory = new ArrayList<String>();
		for (Vendor vendorCategory : listVendor) {
			listVendorCategory.add(vendorCategory.getVendorService());
		}
		listConsumerPrice = consumerPriceRepository
				.findAllIemsByVendorCategory(properties.get("STATUS_QUOTE").toString());

		for (ConsumerPrice objConsumerPrice : listConsumerPrice) {
			for (String vendorSubCategory : listVendorCategory) {
				if (objConsumerPrice.getProduct().getProductSubCategory().equals(vendorSubCategory))
					returnListConsumerPrice.add(objConsumerPrice);
			}
		}
		return returnListConsumerPrice;
	}
	...
	public ConsumerPrice approveUserPrice(long consumerPriceId, String vendorId, List<ConsumerPrice> listConsumerPrice,
			ConsumerPriceRepository consumerPriceRepository, VendorRepository vendorRepository) {
		ConsumerPrice objConsumerPrice = consumerPriceRepository.findByConsumerPriceID(consumerPriceId);
		objConsumerPrice.setStatus(properties.get("STATUS_ACCEPTED").toString());
		java.util.Date currentDate = new java.util.Date();
		objConsumerPrice.setVendor(vendorId);
		objConsumerPrice.setDateAcceptReject(currentDate);
		return consumerPriceRepository.save(objConsumerPrice);
	}
...
/*		List<Long> listProduct = new ArrayList<Long>();
		...
		return consumerPriceRepository.findVendorOrdersByVendorEmail(listVendor.get(0).getVendorEmail(), properties.get("STATUS_ORDER").toString());
	}
	...
	public ConsumerPrice dispatchOrder(String userEmail, long consumerPriceId, List<ConsumerPrice> listConsumerPrice,
		...
		productReview.setConsumerPrice(objConsumerPrice);
