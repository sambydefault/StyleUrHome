...
public interface ConsumerPriceRepository extends JpaRepository<ConsumerPrice, Long> {
	...
	@Query(value = "SELECT * FROM consumer_price cp where cp.status = :status", nativeQuery = true)
	List<ConsumerPrice> findAllIemsByVendorCategory(@Param("status") String status);

	@Query(value = "SELECT * FROM consumer_price cp where cp.vendor = :vendorEmail and cp.status = :status", nativeQuery = true)
	List<ConsumerPrice> findVendorOrdersByVendorEmail(@Param("vendorEmail") String vendorEmail, @Param("status") String status);
