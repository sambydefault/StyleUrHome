...
public interface ConsumerServicePriceRepository extends JpaRepository<ConsumerServicePrice, Long> {
	...
	@Query(value = "SELECT * FROM consumer_service_price cp where cp.status = :status", nativeQuery = true)
	List<ConsumerServicePrice> findAllIemsByVendorCategory(@Param("status") String status);

	@Query(value = "SELECT * FROM consumer_service_price cp where cp.vendor = :vendorEmail and cp.status = :status", nativeQuery = true)
	List<ConsumerServicePrice> findVendorOrdersByVendorEmail(@Param("vendorEmail") String vendorEmail, @Param("status") String status);
